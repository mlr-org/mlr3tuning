test_that("AutoTuner / train+predict", {
  ...
  # lets fake a measure, so we control the best config
  ms = MeasureDummyCPClassif$new(fun = function(pv) if (pv$cp == 0.2) 0 else 1)
...
test_that("AutoTuner hash works #647 in mlr3", {
  ...
  expect_named(
    bmr$resample_result(1)$learners[[1]]$tuning_result,
    c("minsplit", "learner_param_vals", "x_domain", "classif.ce")
  )
  expect_named(
    bmr$resample_result(2)$learners[[1]]$tuning_result,
    c("cp", "learner_param_vals", "x_domain", "classif.ce")
  )
})
...
test_that("AutoTuner errors when train set is not a subset of task ids", {
  ...
  resampling_inner = rsmp("custom")
  resampling_inner$instantiate(task, train_sets = list(11:15), test_sets = list(1:5))
  ...
  resampling_outer$instantiate(task, train_sets = list(1:10), test_sets = list(11:20))
...
test_that("AutoTuner errors when second train set is not a subset of task ids", {
  ...
  resampling_inner = rsmp("custom")
  resampling_inner$instantiate(task, train_sets = list(1:10, 21:30), test_sets = list(31:40, 11:20))
  ...
  resampling_outer = rsmp("custom")
  resampling_outer$instantiate(task, train_sets = list(1:20), test_sets = list(21:40))
...
test_that("AutoTuner errors when test set is not a subset of task ids", {
  ...
  resampling_inner = rsmp("custom")
  resampling_inner$instantiate(task, train_sets = list(1:5), test_sets = list(11:16))
...
test_that("AutoTuner errors when second test set is not a subset of task ids", {
  ...
  resampling_inner = rsmp("custom")
  resampling_inner$instantiate(task, train_sets = list(1:10, 11:20), test_sets = list(11:20, 21:30))
...
test_that("AutoTuner works with internal tuning and validation", {
  ...
  at = auto_tuner(
    ...
    learner = lrn(
      "classif.debug",
      iter = to_tune(1, 1000L, aggr = function(x) length(x)),
      x = to_tune(0.2, 0.3),
      early_stopping = TRUE,
      validate = "test"
    ),
