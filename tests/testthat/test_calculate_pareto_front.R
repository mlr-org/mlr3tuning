...
test_that("calculate_pareto_front", {
  ...
  # maximize
  pareto_front = calculate_pareto_front(data_frame)
  expect_equal(pareto_front, 10)
  # minimize
  pareto_front = calculate_pareto_front(data_frame, maximize = FALSE)
  expect_equal(pareto_front, 1:3)

  # 2D example (slightly turned parallelogram)
  data_frame = data.frame(c(1, 0.25, 0.75, 0), c(0, 0.25, 0.75, 1))
  # maximize
  pareto_front = calculate_pareto_front(data_frame)
