...
TunerIrace = R6Class("TunerIrace", inherit = Tuner,
  ...
  private = list(
    .tune = function(instance) {
      ps = self$param_set$values[!(names(self$param_set$values) %in% c("show.irace.output"))]
      g = if (self$param_set$values$show.irace.output) identity else capture.output # copied from mlr
      g(irace::irace(scenario = c(makeScenario(instance), ps),
