...
#'  * After the batch evaluation, the [Terminator] is checked, and if it is positive,
#'    an exception of class `"terminated_message"` is generated. In this case the current
#'    batch of evaluations is still stored in `instance`, but the numeric scores are not sent back to
...
#'    as the Terminator is only checked after a batch evaluation.
...
#' instance = TuningInstance$new("iris", "classif.rpart", "holdout", "classif.ce", param_set, terminator)
