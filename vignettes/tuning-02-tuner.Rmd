...
Note that the first three points defines the [PerformanceEvaluator] which is already covered in detail in [defining and using performance evaluators]().

In this tutorial, we show how to specify the performance evaluator and terminator, how to conduct the tuning and how to access the tuning result, and how to visualize the hyperparameter tuning effects through several examples.
...
## Specifying the performance evaluator
...
### Collecting everything to the performance evaluator

```{r}
pe = PerformanceEvaluator$new(task, learner, resampling, param_set)
```

For details about the performance evaluator see [defining and using performance evaluators]().
...
This terminator stops after a certain amount of evaluations of the performance evaluator. One evaluation here is one resampling of a parameter combination, and therefore, one evaluation could include multiple trainings of the learner. In our case, with the 3-fold CV, in each evaluation we fit 3 models on each fold.
...
Now that we have specified the performance evaluator and the terminator (that are directly passed to the tuner), we need to choose an optimization algorithm for our parameters that are evaluated by the performance evaluator.
...
```{r}
# pass the performance evaluator (pe) and terminator (tm) to the tuner:
tuner_gs = TunerGridSearch$new(pe, tm, resolution = 15)
```
...
```{r}
# pass the performance evaluator (pe) and terminator (tm) to the tuner:
tuner_rs = TunerRandomSearch$new(pe, tm, batch_size = 5)
```
...
`tune()` simply performs a benchmark on the parameter values generated by the tuner and writes them into a [mlr3::BenchmarkResult] object which is located within the performance evaluator.
```{r}
pe$bmr
```
...
`tune_result()` returns the parameter setting with the best mean performance. As no performance measure was specified, by default the classification error (ce) is used:
```{r}
...
We can get a complete overview about the performed evaluations by calling the member function `aggregated()`:
```{r}
dt_tune = tuner_rs$aggregated()
head(dt_tune)
```
