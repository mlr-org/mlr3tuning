...
```{r}
library(GenSA)
res = GenSA(fn = blackBoxFun, lower = param_set$lowers, upper = param_set$uppers, ff = ff)
```
...
```{r}
res = GenSA(fn = blackBoxFun, lower = param_set$lowers, upper = param_set$uppers,
  control = list(max.call = 120L), ff = ff)
```
...
In `GenSA` we can control the number of iterations by setting the `maxit` argument:
...
To access the real number of iterations we have to take a look at the `nb.steps` column of the trace matrix:

```{r}
head(res$trace.mat)
```

We use the maximum of `nb.steps` as terminator for the iterations. 
...
TunerGenSA = R6Class("TunerGenSA",
  ...
  public = list(
    ...
    initialize = function(ff, evals, ...) {
      if (any(param_set$storage_types != "numeric")) {
  ...
  private = list(
    tune_step = function() {
      ...
      self$GenSA_res = GenSA(fn = blackBoxFun, lower = self$ff$param_set$lowers, upper = self$ff$param_set$uppers,
